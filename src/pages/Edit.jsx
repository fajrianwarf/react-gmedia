...
export default function Edit() {
	...
	const [nama, setNama] = useState(state.nama);
	const [alamat, setAlamat] = useState(state.alamat);
	...
	const getDetail = async () => {
		await axios({
			method: 'post',
			url: '/DemoCase/main/detail_karyawan',
			headers: {
				'User-Id': '1',
			...
			.then((res) => {
				setNama(res.data.response.nama);
				setAlamat(res.data.response.alamat);
				setGender(res.data.response.gender);
				setTgl_lahir(
					res.data.response.tgl_lahir.split('-').reverse().join('-')
				);
			})
	...
	const handleSubmit = async (e) => {
		...
		await axios({
			method: 'post',
			url: '/DemoCase/main/update_karyawan',
			headers: {
				'User-Id': '1',
				'Client-Service': 'gmedia-recruitment',
				'Auth-Key': 'demo-admin',
	...
	return (
		<>
			...
			<div className='container px-5 px-lg-0'>
				<h3 className='mt-5 mb-2'>Edit Data Karyawan</h3>
